...
PALADIN_SKILLS = skill("Bork", SWORDS, MACES, CLOTH, LEATHER, MAIL, PLATE, SHIELD)
...
ALL_SKILLS = (
    ROGUE_SKILLS + PALADIN_SKILLS + MAGE_SKILLS + PRIEST_SKILLS + WARRIOR_SKILLS
)
...
class TheCaveatsTrackTheGaps(unittest.TestCase):
    ...
    def test_the_shield_caveat_goes_with_it(self):
        shield = dict(KAMS, **{"class": 4, "subclass": 6, "inventory_type": 14})
...
class TheSkillMapIsTheCoresOwn(unittest.TestCase):
    ...
    def test_a_shield_needs_the_shield_line(self):
        self.assertEqual(
            recap.item_skill(
                dict(KAMS, **{"class": 4, "subclass": 6, "inventory_type": 14})
            ),
            SHIELD,
        )
    ...
    def test_a_ring_needs_nothing(self):
        self.assertEqual(
            recap.item_skill(
                dict(KAMS, **{"class": 4, "subclass": 0, "inventory_type": 11})
            ),
            0,
        )
    ...
    def test_a_cloak_needs_nothing_though_its_subclass_is_cloth(self):
        ...
        self.assertEqual(
            recap.item_skill(
                dict(KAMS, **{"class": 4, "subclass": 1, "inventory_type": 16})
            ),
            0,
        )
    ...
    def test_a_bag_needs_nothing(self):
        self.assertEqual(
            recap.item_skill(
                dict(KAMS, **{"class": 1, "subclass": 0, "inventory_type": 18})
            ),
            0,
        )
...
class WhatIsNotKnownIsNotARefusal(unittest.TestCase):
    ...
    def test_a_member_absent_from_the_rows_stays_unknown(self):
        ...
        who = {
            m["name"]: m
            for m in recap.family_members(CHARS, WORN, ROSTER, ROGUE_SKILLS)
        }
    ...
    def test_a_skill_row_at_zero_does_not_count_as_holding_it(self):
        ...
        self.assertEqual(recap.verdict(KAMS, who)["verdict"], recap.NO_PROFICIENCY)
...
class TheArmourLadderStillWorksWhereItIsStillUsed(unittest.TestCase):
    ...
    PLATE_CHEST = dict(
        KAMS, **{"class": 4, "subclass": 4, "inventory_type": 5, "required_level": 0}
    )

    def test_with_skills_known_the_answer_is_the_skill_row(self):
        who = member("Og", WARRIOR_SKILLS)  # mail, no plate
        self.assertEqual(
            recap.verdict(self.PLATE_CHEST, who)["verdict"], recap.NO_PROFICIENCY
        )
    ...
    def test_without_skills_the_old_ladder_still_answers(self):
        who = recap.family_members(
            CHARS, [worn("Og", 4, "a mail tunic", 20, subclass=3)], ["Og"]
        )[0]
        self.assertEqual(
            recap.verdict(self.PLATE_CHEST, who)["verdict"], recap.TOO_HEAVY
        )
