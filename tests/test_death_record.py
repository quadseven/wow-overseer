...
MIGRATION = (
    ROOT / "mod-overseer/data/sql/characters/base/2026_08_26_02_overseer_death.sql"
)
...
class DeathHooksDoNoDatabaseWork(unittest.TestCase):
    ...
    def test_none_of_the_hooks_can_null_deref(self):
        ...
        for body in (
            _code(_record_death()),
            _code(_remember_killer()),
            _code(_on_pvp_kill()),
            _code(_on_killed_by_creature()),
        ):
...
class OnlyTheWorldThreadWritesTheTable(unittest.TestCase):
    ...
    def test_only_flush_deaths_inserts_into_the_table(self):
        self.assertEqual(
            1,
            _code(_source()).count("INSERT INTO overseer_death"),
    ...
    def test_every_inserted_column_exists_in_the_create_table(self):
        ...
        for column in self.INSERT_COLUMNS:
            self.assertIn(
                "`%s`" % column,
                migration,
                "%s is written by FlushDeaths but missing from the migration" % column,
            )
