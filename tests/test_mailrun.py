...
class LettersAreFoldedBackTogether(unittest.TestCase):
    ...
    def test_the_soonest_to_expire_comes_first(self):
        ...
        letters = [
            one for one in mailrun.letters_from_rows(LIVE, FAMILY) if one.holder == "Og"
        ]
...
class WhatIsWorthAskingFor(unittest.TestCase):
    ...
    def test_a_cash_on_delivery_letter_keeps_its_attachment(self):
        ...
        can only ever fail."""
        letters = mailrun.letters_from_rows([_row("Og", 51, cod=5000, item=1)], ["Og"])
        plan = mailrun.plan(letters, {"Og": 8})
        self.assertEqual(plan.takes, ())
    ...
    def test_money_is_still_taken_off_a_cash_on_delivery_letter(self):
        ...
        self.assertEqual([mailrun.command(t) for t in takes], ["take-money mail:52"])
...
class TheCommandIsWhatTheDeployedParserAccepts(unittest.TestCase):
    ...
    def test_the_rendered_shapes(self):
        take = mailrun.Take(
            character="Og", verb=mailrun.TAKE_ITEM, mail_id=14, item_guid=777, why=""
        )
        money = mailrun.Take(
            character="Og", verb=mailrun.TAKE_MONEY, mail_id=41, item_guid=0, why=""
        )
    ...
    def test_a_mailbox_is_still_a_gameobject_to_the_executor(self):
        ...
        self.assertIn(
            "GetGoType() == GAMEOBJECT_TYPE_MAILBOX", MODULE.read_text(encoding="utf-8")
        )
...
class TheReachGateIsAskedOfTheTaker(unittest.TestCase):
    ...
    def test_the_family_where_the_realm_measured_them_is_not_in_reach(self):
        ...
        self.assertFalse(travel.spawn_in_reach(GADGETZAN_MAILBOX, standing(), 8))

    def test_a_character_at_the_mailbox_is_in_reach(self):
        at_it = standing(
            pos_x=GADGETZAN_MAILBOX["x"] + 3.0, pos_y=GADGETZAN_MAILBOX["y"]
        )
    ...
    def test_the_threshold_is_a_radius_and_not_a_squared_one(self):
        ...
        to 64."""
        edge = standing(
            pos_x=GADGETZAN_MAILBOX["x"] + 8.0, pos_y=GADGETZAN_MAILBOX["y"]
        )
        just_out = standing(
            pos_x=GADGETZAN_MAILBOX["x"] + 8.01, pos_y=GADGETZAN_MAILBOX["y"]
        )
        inside = standing(
            pos_x=GADGETZAN_MAILBOX["x"] + 4.0, pos_y=GADGETZAN_MAILBOX["y"]
        )
        far = standing(
            pos_x=GADGETZAN_MAILBOX["x"] + 20.0, pos_y=GADGETZAN_MAILBOX["y"]
        )
    ...
    def test_both_axes_count_not_just_one(self):
        ...
        corner = standing(
            pos_x=GADGETZAN_MAILBOX["x"] + 6.0, pos_y=GADGETZAN_MAILBOX["y"] + 6.0
        )
    ...
    def test_a_spawn_on_another_map_is_never_in_reach(self):
        ...
        one step earlier."""
        elsewhere = standing(
            map_id=530, pos_x=GADGETZAN_MAILBOX["x"], pos_y=GADGETZAN_MAILBOX["y"]
        )
    ...
    def test_an_unreadable_reading_is_refused_rather_than_raising(self):
        ...
        self.assertFalse(
            travel.spawn_in_reach(GADGETZAN_MAILBOX, standing(pos_x="over there"), 8)
        )
        self.assertFalse(
            travel.spawn_in_reach(dict(GADGETZAN_MAILBOX, map_id=None), standing(), 8)
        )
        self.assertFalse(
            travel.spawn_in_reach(GADGETZAN_MAILBOX, standing(map_id=None), 8)
        )
    ...
    def test_the_caller_owns_the_threshold(self):
        ...
        out_at_eight = standing(
            pos_x=GADGETZAN_MAILBOX["x"] + 20.0, pos_y=GADGETZAN_MAILBOX["y"]
        )
        self.assertFalse(travel.spawn_in_reach(GADGETZAN_MAILBOX, out_at_eight, 8))
        self.assertTrue(travel.spawn_in_reach(GADGETZAN_MAILBOX, out_at_eight, 25))
    ...
    def test_there_is_one_of_it_and_its_name_names_no_counter(self):
        ...
        for copy in (
            "def vault_in_reach(",
            "def mailbox_in_reach(",
            "def forge_in_reach(",
            "def counter_in_reach(",
        ):
...
class AMailRowIsAnsweredWhereTheCharacterStandsTests(unittest.TestCase):
    ...
    def test_a_mailbox_is_required_before_any_verb_branches(self):
        ...
        for verb in ("MailVerb::Send", "MailVerb::TakeItem", "MailVerb::TakeMoney"):
            with self.subTest(verb=verb):
                self.assertLess(gate, self.body.index("req.verb == D::%s" % verb))
    ...
    def test_retryable_is_carried_out_to_the_sender_and_moves_no_row(self):
        ...
        described = self.body[
            self.body.index("auto describe = [&]") : self.body.index(
                "auto refuse = [&]"
            )
        ]
