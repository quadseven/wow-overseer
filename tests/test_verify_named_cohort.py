...
class VerifierTest(unittest.TestCase):
    ...
    def test_duplicate_account_is_rejected(self):
        ...
        self.assertTrue(any("separate accounts" in error
                            for error in MODULE.verify_characters(rows, MODULE.BONKERS)))

    def test_realm_identity_is_checked(self):
        self.assertEqual([], MODULE.verify_realm([{"id": 1, "name": "Homelab-Dev"}],
                                                  1, "Homelab-Dev"))
        self.assertTrue(MODULE.verify_realm([{"id": 2, "name": "Homelab-Dev"}],
                                            1, "Homelab-Dev"))

    def test_guild_count_requires_eligible_random_bots(self):
        rows = [{"guild_name": "Cave", "random_bot_members": 50},
    ...
    def test_queries_are_select_only(self):
        ...
        writes = re.findall(r"execute\(\s*[\"'](INSERT|UPDATE|DELETE)", body,
                            flags=re.IGNORECASE)
