...
TOOL = (
    pathlib.Path(__file__).resolve().parents[1] / "tools" / "gather_bands_from_dbc.py"
)


def spawn(zone, lock, map_id=1, x=0.0, y=0.0, z=0.0, name=""):
    return gatheraim.Spawn(
        map_id=map_id, zone_id=zone, x=x, y=y, z=z, lock_id=lock, name=name
    )
...
class TheProjectionMatchesItsGenerator(unittest.TestCase):
    ...
    def test_the_tool_exists_and_documents_the_dbc_checksum(self):
        ...
        self.assertIn(
            "74ddd0458e50a1d65acb78700b45e4d1",
            source,
    ...
    def test_the_tool_refuses_a_file_that_is_not_a_dbc(self):
        out = subprocess.run(
            [sys.executable, str(TOOL), str(TOOL)], capture_output=True, text=True
        )
...
class TheFamilyTravelsAsOne(unittest.TestCase):
    ...
    def test_a_roster_holding_no_aimable_skill_says_so(self):
        got = gatheraim.choose(
            skills={"Bork": {"skinning": 12}},
            standing_on=1,
            spawns=[spawn(148, COPPER)],
            family_level=60,
            zone_levels={148: 10},
        )
...
class TheDestinationIsAlwaysASurveyedRow(unittest.TestCase):
    ...
    def test_the_chosen_coordinate_is_one_of_the_input_spawns(self):
        rows = [
            spawn(148, COPPER, x=100.0, y=100.0, z=7.5),
    ...
    def test_the_centroid_itself_is_never_returned(self):
        ...
        rows = [
            spawn(148, COPPER, x=0.0, y=0.0, z=5.0),
        ...
        self.assertIn(
            (got.chosen.spawn.x, got.chosen.spawn.z), [(0.0, 5.0), (100.0, 50.0)]
        )
