...
# The family: all bots, all guild 7.
ROSTER = [_row("Grug", 6), _row("Ugga", 5), _row("Grog", 6),
          _row("Bork", 4), _row("Og", 5)]
...
class ParsePleaTest(unittest.TestCase):
    ...
    def test_ordinary_chatter_is_not_a_plea(self):
        for line in ("Grug smash boars", "nice loot", "I helped Ugga earlier",
                     "no help needed", "helpful little gnome"):
    ...
    def test_help_as_a_mere_prefix_is_not_a_plea(self):
        ...
        for line in ("I need helpful directions to Orgrimmar",
    ...
    def test_negated_help_is_not_a_plea(self):
        ...
        for line in ("you can't help me now", "nobody can help me",
                     "no one can help me", "the innkeeper wouldn't help me",
                     "I don't think I need help", "I never said I need help"):
            self.assertIsNone(kin.parse_plea("Grug", line), line)

    def test_being_told_to_stop_helping_is_not_a_plea(self):
        """Fails without _DISMISS_RE."""
        for line in ("stop trying to help me", "quit trying to help me",
...
class PlanMusterTest(unittest.TestCase):
    ...
    def test_the_family_answers_and_the_caller_does_not_answer_itself(self):
        m = kin.plan_muster(self._plea(), ROSTER, last_muster_at=None, now=100.0)
    ...
    def test_a_caller_nobody_knows_musters_nobody(self):
        m = kin.plan_muster(self._plea(caller="Stranger"), ROSTER,
                            last_muster_at=None, now=100.0)
        self.assertEqual(m.actions, [])
        self.assertIn("not one of", m.reason)

    def test_a_second_plea_inside_the_cooldown_is_refused(self):
        m = kin.plan_muster(self._plea(), ROSTER, last_muster_at=90.0, now=100.0)
        self.assertEqual(m.actions, [])
        self.assertIn("cooldown", m.reason)

    def test_the_cooldown_expires(self):
        m = kin.plan_muster(self._plea(), ROSTER,
                            last_muster_at=100.0 - kin.COOLDOWN_SECONDS - 1, now=100.0)
        self.assertTrue(m.actions)

    def test_a_lone_character_musters_nobody_rather_than_erroring(self):
        m = kin.plan_muster(self._plea(caller="Grug"), [_row("Grug", 6)],
                            last_muster_at=None, now=100.0)
        self.assertEqual(m.actions, [])
    ...
    def test_responders_are_capped(self):
        ...
        m = kin.plan_muster(self._plea(), big, last_muster_at=None, now=100.0)
    ...
    def test_a_stranger_in_another_guild_is_never_mustered(self):
    ...
    def test_a_guildless_caller_musters_nobody_rather_than_the_realm(self):
        # Grog appears ONCE, guildless. Prepending him to ROSTER would not
        # work: plan_muster keys by name, so ROSTER's guild-7 Grog wins.
        roster = [_row("Grog", 6, guild_id=0)] + [r for r in ROSTER if r["name"] != "Grog"]
        m = kin.plan_muster(kin.Plea("Grog", ""), roster, last_muster_at=None, now=100.0)
        self.assertEqual(m.actions, [])
        self.assertIn("no guild", m.reason)
...
class ReportTest(unittest.TestCase):
    ...
    def test_a_report_names_who_regrouped(self):
        m = kin.plan_muster(kin.Plea("Grog", "my paladin quest"), ROSTER,
                            last_muster_at=None, now=100.0)
    ...
    def test_a_report_never_claims_help_reached_the_caller(self):
        ...
        m = kin.plan_muster(kin.Plea("Grog", ""), ROSTER,
                            last_muster_at=None, now=100.0)
...
class MemoryTest(unittest.TestCase):
    ...
    def test_memory_survives_an_empty_about(self):
        m = kin.plan_muster(kin.Plea("Bork", ""), ROSTER,
                            last_muster_at=None, now=100.0)
