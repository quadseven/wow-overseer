    ...
    def test_two_empty_positions_take_the_two_biggest_in_order(self):
        moves = fill_empty_positions(
            ...
            [
                ...
                Bag("Red Leather Bag", 12),
                Bag("Green Leather Bag", 10),
            ],
        )
        self.assertEqual(
            [m.bag for m in moves], ["Red Leather Bag", "Green Leather Bag"]
        )
...
class ASwapMustNotStrandWhatComesOut(unittest.TestCase):
    ...
    def test_a_strictly_larger_bag_replaces_a_small_one(self):
        moves = upgrade_swaps(
            [Bag("Small Black Pouch", 6, used=0)],
            [Bag("Red Leather Bag", 12)],
            free_slots=0,
        )
        self.assertEqual(len(moves), 1)
    ...
    def test_the_same_swap_is_allowed_once_there_is_room_to_land(self):
        moves = upgrade_swaps(
            [Bag("Small Black Pouch", 6, used=6)], [Bag("Linen Bag", 8)], free_slots=4
        )
        self.assertEqual(len(moves), 1)
...
class ThePlanAsAWhole(unittest.TestCase):
    ...
    def test_every_move_carries_its_reason(self):
        for move in plan_bag_moves(4, BORK_WORN, GROG_CARRIED, free_slots=20):
            self.assertTrue(
                move.why.strip(), "a move with no reason cannot be reviewed in a log"
            )
...
FAMILY = [
    Member(
        "Grug",
        4,
        worn=tuple(Bag("worn", 16, used=16, guid=900 + i) for i in range(4)),
        carried=(
            Bag("Small Red Pouch", 6, guid=11),
    ...
    Member(
        "Ugga",
        4,
        worn=tuple(Bag("worn", 10, used=10, guid=910 + i) for i in range(4)),
        carried=(),
    ),
    Member(
        "Og",
        4,
        worn=tuple(Bag("worn", 14, used=14, guid=920 + i) for i in range(4)),
        carried=(Bag("Small Black Pouch", 6, guid=21),),
    ),
    Member(
        "Grog",
        4,
        worn=tuple(Bag("worn", 11, used=11, guid=930 + i) for i in range(4)),
        carried=(
            Bag("Red Leather Bag", 12, guid=31),
    ...
    Member(
        "Bork",
        4,
        worn=tuple(Bag("worn", 13, used=13, guid=940 + i) for i in range(3)),
        carried=(),
    ),
]
...
class NothingIsPlannedTwice(unittest.TestCase):
    def test_two_empty_positions_get_two_different_bags(self):
        ...
        family[-1] = Member(
            "Bork",
            4,
            worn=tuple(Bag("worn", 13, used=13, guid=940 + i) for i in range(2)),
            carried=(),
        )
    ...
    def test_nobody_gives_a_bag_to_themselves(self):
        ...
        alone = [
            Member(
                "Grug",
                4,
                worn=(Bag("worn", 16, used=16, guid=900),),
...
class ThePlanIsStable(unittest.TestCase):
    ...
    def test_member_order_does_not_change_the_answer(self):
        self.assertEqual(
            plan_family_bags(FAMILY), plan_family_bags(list(reversed(FAMILY)))
        )
...
class RowsBecomeMembers(unittest.TestCase):
    ...
    def test_strangers_rows_are_ignored(self):
        self.assertEqual(
            [m.name for m in members_from_rows(self.ROWS, ["Grug"])], ["Grug"]
        )
    ...
    def test_the_measured_family_plans_from_rows_the_same_as_from_members(self):
        ...
        rows = [
            ...
            _row("Bork", 940, "worn", 13, 0, 19),
            _row("Bork", 941, "worn", 13, 0, 20),
            _row("Bork", 942, "worn", 13, 0, 21),
        ]
        moves = plan_family_bags(members_from_rows(rows, ["Bork", "Grog"]))
        self.assertEqual(
            [(m.giver, m.receiver, m.guid) for m in moves], [("Grog", "Bork", 32)]
        )
    ...
    def test_nonempty_carried_bags_are_not_offered(self):
        ...
        self.assertEqual(
            plan_family_bags(members_from_rows(rows, ["Bork", "Grog"])), []
        )
