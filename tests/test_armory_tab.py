...
class WhereTheCodeIsAllowedToSit(unittest.TestCase):
    ...
    def test_the_styles_sit_above_the_family_banner(self):
        ...
        self.assertLess(
            self.page.index("--- the Armory tab (infra#3096, infra#3139)"),
...
class TheArmoryTab(unittest.TestCase):
    ...
    def test_a_third_tab_can_actually_light_up(self):
        ...
        self.assertNotIn("FAMILY_VIEW", mark, "markTabs must not know any view by name")
    ...
    def test_the_map_is_hidden_behind_every_view_that_is_not_the_map(self):
        ...
        self.assertIn(
            'wrapEl.style.display = view === MAP_VIEW ? "block" : "none";', show
        )
    ...
    def test_the_doll_layout_is_not_retyped_into_the_page(self):
        ...
        self.assertIn(
            "renderMember(armoryProfile(name, slot, false), one.member, one.doll)",
            self.tab,
        )
    ...
    def test_the_page_leaves_the_tailnet_for_exactly_two_things(self):
        ...
        self.assertIn(
            'const MODEL_SCRIPT = "https://wow.zamimg.com/modelviewer/wrath/'
            "deployment/viewer/",
            self.tab,
        )
        ...
        self.assertIn(
            "img.onerror = () => { img.remove(); if (onFail) onFail(); };", self.tab
        )
    ...
    def test_the_model_is_an_upgrade_over_the_portrait_never_a_replacement(self):
        ...
        than removed."""
        self.assertIn(
            "const timer = setTimeout(() => { script.remove(); resolve(false); },"
            " MODEL_LOAD_TIMEOUT);",
            self.tab,
        )
        ...
        self.assertIn(
            ".aportrait.live .race, .aportrait.live .sil, .aportrait.live .cls "
            "{ display:none; }",
            self.css,
        )
    ...
    def test_the_face_is_the_worlds_five_numbers_matched_by_option_name(self):
        ...
        self.assertIn(
            'if (optionName === "Horn Style" && !names.includes("Hair Style")) '
            'return "hairStyle";',
            self.tab,
        )
    ...
    def test_the_item_card_rides_on_the_model_and_not_in_the_flow(self):
        ...
        self.assertIn(
            ".acard { position:absolute; left:0; right:0; bottom:0;", self.acss
        )
        for gone in (
            "pointerenter",
            "arm.pinned",
    ...
    def test_the_card_cannot_leave_the_stage_or_the_screen(self):
        ...
        self.assertIn("overflow:hidden", rule, "the stage must clip the card it holds")
    ...
    def test_the_card_is_capped_by_the_stage_and_not_by_a_fraction_of_it(self):
        ...
        self.assertNotIn(
            "min(60%", rules, "a fraction of a stage that is now the model's own box"
        )
        self.assertNotIn(
            "height:100%",
            rules,
    ...
    def test_the_stage_is_a_shape_and_not_whatever_its_row_happens_to_be(self):
        ...
        self.assertNotIn(
            "min-height:300px", rule, "a floor under a ratio only letterboxes the model"
        )
    ...
    def test_the_canvas_is_given_both_its_sides_and_not_only_its_width(self):
        ...
        self.assertIn(
            ".amodel canvas { display:block; width:100%; height:100%; }", self.acss
        )
        self.assertIn(
            ".aportrait.live .amodel { display:block; height:100%; }", self.acss
        )
    ...
    def test_the_canvas_is_fitted_to_the_stage_and_carries_its_ratio(self):
        ...
        self.assertIn(
            "new ResizeObserver(() => fitModel(c)).observe(c.portrait);", self.ajs
        )
        self.assertNotIn(
            "MODEL_ASPECT)",
            self.ajs[
                self.ajs.index("function fitModel") : self.ajs.index(
                    "function watchModel"
                )
            ],
    ...
    def test_the_card_goes_away_every_way_a_tap_can_ask(self):
        ...
        self.assertIn(
            'if (t instanceof Element && t.closest(".acard, .aslot")) return;', self.ajs
        )
    ...
    def test_the_cell_says_the_card_is_open_and_says_which_one(self):
        ...
        self.assertIn(
            'cell.setAttribute("aria-expanded", String(slot === c.selected));', self.tab
        )
    ...
    def test_the_hint_stays_in_the_flow_and_never_moves(self):
        ...
        self.assertIn(
            "prof.append(head, body, hint, spec, bar, trees, unplaced);", self.tab
        )
    ...
    def test_the_card_is_readable_over_a_moving_render(self):
        ...
        rules = rules[: rules.index(".acard[hidden]")]
        self.assertLess(
            rules.index("background:var(--ink);"), rules.index("background:color-mix")
        )
    ...
    def test_choosing_a_slot_rings_it_and_choosing_again_lets_go(self):
        ...
        self.assertIn(
            ".aslot.broken { box-shadow:0 0 0 2px var(--vermilion); }", self.acss
        )
    ...
    def test_the_card_draws_the_lines_the_game_draws(self):
        ...
        tip = self.tab[
            self.tab.index("function renderDetail") : self.tab.index(
    ...
    def test_an_unavailable_stat_says_so_and_the_page_cannot_say_zero(self):
        ...
        stats = self.tab[
            self.tab.index("function renderStats") : self.tab.index(
                "function showTrees"
            )
        ]
        ...
        self.assertNotIn(
            '"unavailable"', self.ajs, "the page must not name a stat's source itself"
        )
    ...
    def test_a_stat_is_coloured_by_where_its_number_came_from(self):
        ...
        for source in (armory.STAT_SAVED, armory.STAT_DERIVED, armory.STAT_UNAVAILABLE):
    ...
    def test_the_talent_trees_are_collapsed_until_they_are_asked_for(self):
        ...
        self.assertFalse(armory.TREES_EXPANDED, "the trees must not start open")
    ...
    def test_a_shut_tree_is_not_built_at_all(self):
        ...
        spec = self.tab[
            self.tab.index("function showTrees") : self.tab.index("function renderSpec")
        ]
    ...
    def test_the_page_does_not_name_the_toggle_or_the_summary(self):
        ...
        self.assertIn(
            "c.toggle.textContent = c.open ? arm.trees.hide : arm.trees.show;", self.tab
        )
    ...
    def test_the_unspent_points_are_the_amber_half_of_the_bar(self):
        ...
        self.assertIn(
            ".aspecbar .unspent { background:var(--caution-text); }", self.acss
        )
        self.assertIn(".aspecbar .spent { background:var(--accent-text); }", self.acss)
    ...
    def test_the_headline_verdict_is_not_a_ternary_in_the_page(self):
        ...
        self.assertIn(
            'aheadline.className = p.headline.alarm ? "alarm" : "";', self.tab
        )
    ...
    def test_a_card_with_no_character_behind_it_shows_no_empty_controls(self):
        ...
        self.assertIn(
            ".aprof.c-gone .abody, .aprof.c-gone .ahint, .aprof.c-gone .aspec,",
            self.acss,
        )
        self.assertIn(
            ".aprof.c-gone .aspecbar, .aprof.c-gone .atrees { display:none; }",
            self.acss,
        )
    ...
    def test_the_talent_grid_is_the_trainers(self):
        ...
        self.assertIn(
            'r.textContent = t.rank ? t.rank + "/" + t.max_rank : "";', self.tab
        )
    ...
    def test_a_cosmetic_empty_slot_is_quieter_than_a_real_one(self):
        ...
        self.assertIn(
            ".aslot.empty.cosmetic { border-color:var(--on-dark-faint); "
            "color:var(--on-dark-faint); }",
            self.acss,
        )
        self.assertIn(
            ".aslot.empty:not(.cosmetic) { border-color:var(--caution-text);", self.acss
        )
    ...
    def test_one_profile_on_a_phone_and_two_on_a_wide_desktop(self):
        ...
        self.assertIn(
            "#asides, .apair { display:grid; gap:1rem; grid-template-columns:minmax(0,1fr); }",
            self.acss,
        )
        self.assertIn(
...
class TheTwoFamiliesAndTheGuilds(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.ajs = tab[
            : tab.index("// --- the standing panel (mod-overseer#88, mod-overseer#160)")
        ]
        cls.page = page
        cls.server = (HERE / "map_server.py").read_text()
    ...
    def test_the_section_has_a_place_for_the_sides_and_the_guilds(self):
        ...
        self.assertLess(
            armory_html.index('id="aprofiles"'), armory_html.index('id="aguilds"')
        )
    ...
    def test_the_rows_come_from_the_payloads_pairs(self):
        ...
        self.assertIn(
            "placeProfile(row.right, pair.right, p.no_counterpart);", self.ajs
        )
    ...
    def test_a_guild_is_closed_by_default_and_fetched_only_when_opened(self):
        ...
        self.assertIn(
            'u("/api/armory/guild?guild=" + encodeURIComponent(s.name))', self.ajs
        )
