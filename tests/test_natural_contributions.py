...
class TheRule(unittest.TestCase):
    ...
    def test_the_family_contributes_unless_it_holds_unearned_dues(self):
        ...
        self.assertIn(
            "discard-unearned-gold", natural.why_not("Grug", FAMILY, {}, ["Grug"])
        )
    ...
    def test_an_unreadable_ledger_resets_nobody(self):
        parts = ledger(("Goraraa", "reset"))
        got = natural.contributors(
            ["Goraraa", "Bork"], FAMILY, parts, ["Grug"], ledger_readable=False
        )
...
class TheDues(unittest.TestCase):
    ...
    def members(self):
        return [
            guildwork.Member("Goraraa", "Cave", 3254 * GOLD, True),
    ...
    def test_a_member_not_reset_posts_nothing(self):
        ...
        plan = guildwork.plan_dues(
            crew,
            self.masters,
            {m.name: walker(m.name) for m in crew},
            (),
            (),
            eligible={"Goraraa"},
        )
...
class TheBridgeIsWired(unittest.TestCase):
    def test_the_dues_pass_asks_the_ledger_and_passes_the_gate(self):
        self.assertIn(
            "_natural_contributors, [m.name for m in members], names)", BRIDGE
        )
        self.assertIn(
    ...
    def test_a_tab_is_bought_only_with_earned_gold(self):
        ...
        self.assertIn(
            "if master not in _natural_contributors([master], names):", BRIDGE
        )
