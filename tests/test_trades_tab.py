...
FETCH = SERVER[SERVER.index("_TRADE_SKILL_LIST = "):
...
class WhereTheCodeIsAllowedToSit(unittest.TestCase):
    ...
    def test_the_fetch_sits_above_both_fetch_windows_that_forbid_a_bare_read(self):
        ...
        self.assertLess(SERVER.index("def _fetch_guildcraft"),
    ...
    def test_the_handler_sits_in_the_gap_between_two_claimed_windows(self):
        ...
        self.assertGreater(SERVER.index("def _trades"),
                           SERVER.index("def _thoughts"))
        self.assertLess(SERVER.index("def _trades"),
                        SERVER.index("def _dungeons"))


class TheMarkup(unittest.TestCase):
    def test_the_section_exists_beside_the_other_views(self):
        for element in ('id="gcrhead"', 'id="gcrguild"', 'id="gcrcoverage"',
                        'id="gcrgapsline"', 'id="gcrgaps"', 'id="gcrorder"',
                        'id="gcrlist"', 'id="gcrbasis"'):
    ...
    def test_the_gaps_are_above_the_list_and_not_below_it(self):
        ...
        self.assertLess(SECTION.index('id="gcrgaps"'),
    ...
    def test_the_coverage_count_is_above_the_list_too(self):
        ...
        reaching the bottom."""
        self.assertLess(SECTION.index('id="gcrcoverage"'),
                        SECTION.index('id="gcrlist"'))
    ...
    def test_the_rule_it_was_ordered_by_is_above_the_list(self):
        ...
        self.assertLess(SECTION.index('id="gcrorder"'),
                        SECTION.index('id="gcrlist"'))

    def test_the_basis_is_below_the_list_it_describes(self):
        self.assertGreater(SECTION.index('id="gcrbasis"'),
                           SECTION.index('id="gcrlist"'))
    ...
    def test_the_markup_holds_no_sentence_of_its_own_about_a_trade(self):
        ...
        for invented in ("engineering", "recipe items", "in reach",
                         "short", "trainer", "vendor"):
...
class ThePoll(unittest.TestCase):
    def test_it_stops_when_the_tab_is_not_open(self):
        ...
        self.assertIn("if (view === TRADES_VIEW) pollTrades(); }, 120000)",
                      CODE)
...
class TheTabAndItsAddress(unittest.TestCase):
    ...
    def test_it_sits_after_the_dungeon_plan_and_before_the_continents(self):
        ...
        self.assertLess(PAGE.index("tabs.appendChild(tb);"),
    ...
    def test_showview_toggles_the_section(self):
        ...
        self.assertIn('gcrsection.style.display = isTrd ? "block" : "none";',
                      show)
...
class TheReads(unittest.TestCase):
    def test_every_world_read_is_guarded_for_both_errors(self):
        ...
        for table in ("guild_member", "characters", "character_skills",
                      "character_spell", "overseer_roster", "item_template",
                      "trainer_spell", "npc_vendor", "creature_loot_template",
                      "quest_template"):
    ...
    def test_nothing_is_read_once_per_trade_or_once_per_recipe(self):
        ...
        body = FETCH[FETCH.index("with conn.cursor()"):]
        for once in ("_TRADE_RECIPES", "_TRADE_VENDORS", "_TRADE_DROPS",
                     "_TRADE_QUESTS", "_TRADE_TRAINER"):
    ...
    def test_the_bound_skill_ids_are_the_modules_and_are_not_user_input(self):
        ...
        self.assertEqual(sorted(guildcraft.recipe_skills()),
                         guildcraft.recipe_skills())
...
class TheHouseRules(unittest.TestCase):
    def test_no_em_dashes(self):
        for name in ("index.html", "map_server.py", "guildcraft.py",
