...
def recipe(
    ...
    return {
        ...
        "item_name": name,
        "RequiredSkill": ident,
        "RequiredSkillRank": rank,
        "required_level": level,
        "spellid_2": spell,
        "quality": quality,
        "item_level": rank,
        "displayid": 1,
        "class": guildcraft.RECIPE_CLASS,
        "subclass": 2,
    }


def vendor(item: int, name: str, x=WESTFALL_X, y=WESTFALL_Y, map_id: int = 0) -> dict:
    return {"item": item, "name": name, "map": map_id, "position_x": x, "position_y": y}
...
FAMILY_SKILLS = [
    skill("Grug", BLACKSMITHING, 1),
    skill("Grug", MINING, 5),
    skill("Bork", goals.SKILL_IDS["leatherworking"], 1),
    skill("Bork", goals.SKILL_IDS["skinning"], 12),
    skill("Og", TAILORING, 20),
    skill("Og", ENCHANTING, 1),
    skill("Ugga", ALCHEMY, 30),
    skill("Ugga", HERBALISM, 34),
]
...
class TheCoveredRoster(unittest.TestCase):
    ...
    def test_a_guild_widens_the_roster_and_is_named(self):
        rows = [
            {"guild": "The Long Table", "name": name}
            for name in ROSTER + ["Kam", "Viper"]
        ]
        self.assertEqual(
            guildcraft.covered_names(ROSTER, rows), ROSTER + ["Kam", "Viper"]
        )
        payload = build(guild_rows=rows, member_rows=[member(n) for n in ROSTER])
    ...
    def test_an_uncut_roster_does_not_warn(self):
        self.assertIn("Every character this page covers was read", build()["coverage"])
...
class WhoHoldsWhat(unittest.TestCase):
    ...
    def test_every_trade_this_service_names_gets_a_row(self):
        payload = build()
        self.assertEqual({t["name"] for t in payload["trades"]}, set(goals.SKILL_IDS))

    def test_the_holder_line_carries_the_ceiling_and_not_just_the_value(self):
        """1 of 75 and 1 of 300 are different characters."""
        self.assertIn(
            "Og holds tailoring at 20 of 75", trade(build(), "tailoring")["line"]
        )
    ...
    def test_a_skill_at_its_ceiling_says_so(self):
        ...
        self.assertIn(
            "as far as their training goes", trade(payload, "tailoring")["line"]
        )
...
class WhatNobodyHolds(unittest.TestCase):
    ...
    def test_a_trade_nobody_holds_still_counts_its_recipes(self):
        ...
        rows = [
            recipe(n, "Schematic %d" % n, ENGINEERING, 20, 800 + n) for n in range(3)
        ]
    ...
    def test_a_trade_assigned_and_not_held_is_a_different_gap(self):
        ...
        payload = build(
            skill_rows=[],
            roster_rows=[
                {"name": "Og", "professions": "%d,%d" % (TAILORING, ENCHANTING)}
            ],
        )
    ...
    def test_an_assignment_the_world_agrees_with_is_not_a_complaint(self):
        payload = build(roster_rows=[{"name": "Og", "professions": str(TAILORING)}])
        self.assertEqual(
            trade(payload, "tailoring")["assigned_line"],
...
class WhereAMissingRecipeComesFrom(unittest.TestCase):
    ...
    def test_a_vendor_is_named_and_placed_in_a_real_zone(self):
        payload = build(
            recipe_rows=self.rows, vendor_rows=[vendor(70, "Jannos Ironwill")]
        )
        card = trade(payload, "tailoring")["missing"][0]
        self.assertEqual(card["sources"][0]["kind"], guildcraft.VENDOR)
        self.assertIn(
            "sold by Jannos Ironwill in %s" % WESTFALL, card["sources"][0]["line"]
        )

    def test_a_drop_carries_the_creature_its_levels_and_where(self):
        payload = build(recipe_rows=self.rows, drop_rows=[drop(70, "Defias Conjurer")])
    ...
    def test_a_drop_inside_a_dungeon_is_named_by_the_dungeon(self):
        ...
        payload = build(
            recipe_rows=self.rows, drop_rows=[drop(70, "Mr. Smite", map_id=36)]
        )
        line = trade(payload, "tailoring")["missing"][0]["sources"][0]["line"]
    ...
    def test_a_quest_is_named_with_the_level_it_is_written_for(self):
        payload = build(
            recipe_rows=self.rows, quest_rows=[quest(70, "Red Linen Goods")]
        )
        line = trade(payload, "tailoring")["missing"][0]["sources"][0]["line"]
    ...
    def test_the_sources_are_ordered_by_what_they_cost_to_reach(self):
        ...
        kinds = [
            s["kind"] for s in trade(payload, "tailoring")["missing"][0]["sources"]
        ]
        self.assertEqual(kinds, [guildcraft.VENDOR, guildcraft.QUEST, guildcraft.DROP])
    ...
    def test_no_source_read_is_not_no_source(self):
        ...
        self.assertIn("world drop behind a reference loot table", card["source_line"])

    def test_a_long_source_list_is_trimmed_and_says_how_many_there_were(self):
        drops = [drop(70, "Defias %d" % n) for n in range(9)]
        card = trade(build(recipe_rows=self.rows, drop_rows=drops), "tailoring")[
            "missing"
        ][0]
...
class WhatItWouldTakeToLearnIt(unittest.TestCase):
    ...
    def test_a_recipe_above_one_holders_training_names_the_trainer(self):
        ...
        payload = build(
            ...
            skill_rows=[
                skill("Og", TAILORING, 20, ceiling=150),
    ...
    def test_a_character_level_gate_is_named_separately_from_skill(self):
        ...
        payload = build(
            recipe_rows=[recipe(70, "Pattern", TAILORING, 10, 900, level=50)]
        )
        card = trade(payload, "tailoring")["missing"][0]
    ...
    def test_recipes_above_the_ceiling_are_counted_and_not_listed(self):
        ...
        rows = [
            recipe(n, "Pattern %d" % n, TAILORING, 10, 900 + n) for n in range(2)
        ] + [
            recipe(50 + n, "Pattern %d" % n, TAILORING, 200, 950 + n) for n in range(7)
        ]
        card = trade(build(recipe_rows=rows), "tailoring")
        self.assertEqual(card["missing_count"], 2)
        self.assertIn(
            "7 more recipe items exist for it above skill 75", card["beyond_line"]
        )
    ...
    def test_the_reach_is_measured_against_the_best_ground_skill(self):
        ...
        self.assertIn(
            "Grog has 45", trade(payload, "tailoring")["missing"][0]["reach_line"]
        )
...
class ThingsItRefusesToClaim(unittest.TestCase):
    ...
    def test_a_trainer_craft_already_known_is_counted_as_known(self):
        ...
        card = trade(
            build(trainer_rows=trainer, spell_rows=[{"name": "Og", "spell": 2963}]),
            "tailoring",
        )
    ...
    def test_a_trimmed_missing_list_says_how_many_there_were(self):
        rows = [recipe(n, "Pattern %d" % n, TAILORING, 10, 900 + n) for n in range(30)]
        card = trade(build(recipe_rows=rows), "tailoring")
...
class EveryCountCanBeOne(unittest.TestCase):
    ...
    def test_a_single_source_row_is_a_row_and_not_rows(self):
        payload = build(
            recipe_rows=[recipe(70, "Pattern", TAILORING, 10, 900)],
            vendor_rows=[vendor(70, "Jannos Ironwill")],
        )
    ...
    def test_the_recipes_line_reads_as_a_sentence_in_all_three_states(self):
        ...
        rows = [recipe(n, "Pattern %d" % n, TAILORING, 10, 900 + n) for n in range(4)]
        some = trade(build(recipe_rows=rows), "tailoring")["recipes_line"]
        self.assertIn("4 more sit under the training already held, all listed", some)
        rows = [recipe(n, "Pattern %d" % n, TAILORING, 10, 900 + n) for n in range(30)]
        many = trade(build(recipe_rows=rows), "tailoring")["recipes_line"]
        self.assertIn(
            "the %d cheapest to reach are listed" % guildcraft.LISTED_MISSING, many
        )
...
class TheOrderAndThePlaces(unittest.TestCase):
    ...
    def test_a_sourced_recipe_is_listed_before_an_unsourced_one(self):
        rows = [
            recipe(1, "Aaa Pattern", TAILORING, 10, 901),
            recipe(2, "Zzz Pattern", TAILORING, 10, 902),
        ]
        payload = build(recipe_rows=rows, vendor_rows=[vendor(2, "Jannos Ironwill")])
        self.assertEqual(
            [r["entry"] for r in trade(payload, "tailoring")["missing"]], [2, 1]
        )
...
class TheHouseRules(unittest.TestCase):
    ...
    def test_the_skill_ids_are_not_restated_here(self):
        ...
        for number in (164, 165, 171, 197, 202, 333, 755, 773):
            ...
            self.assertIsNone(re.search(r"%d" % number, MODULE), str(number))
