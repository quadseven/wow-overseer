...
class BothReadersUseIt(unittest.TestCase):
    def test_the_vendor_query_no_longer_calls_every_class_12_stack_a_quest_item(self):
        ...
        block = src[
            src.index("_VENDOR_ITEMS_SQL = (") : src.index("_SURPLUS_GEAR_SQL = (")
        ]
    ...
    def test_the_bags_page_reads_the_same_answer(self):
        ...
        self.assertIn(
            '_WEALTH_QUEST_NEEDED = ", " + bag_pressure.QUEST_NEEDED_SQL'
            ' + " AS quest_needed "',
            src,
        )
        fetch = src[
            src.index("def _fetch_wealth") : src.index("# Everything a tooltip draws")
        ]
        self.assertTrue(
            re.search(r'f"\{_WEALTH_ITEM_COLUMNS\}\{_WEALTH_QUEST_NEEDED\}"', fetch)
        )
