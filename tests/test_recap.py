...
ITEMS = {
    2169: {"name": "Buzzer Blade", "Quality": 2, "ItemLevel": 22,
           "displayid": 1},
    10413: {"name": "Gloves of the Fang", "Quality": 3, "ItemLevel": 24,
            "displayid": 2},
}


def equip(who, entry, name, when, map_id=WAILING, zone=718, occurrences=1,
          detail="slot 9"):
    return {"character_name": who, "kind": "item_equip", "subject_id": entry,
            "subject_name": name, "detail": detail, "map": map_id,
            "zone": zone, "occurrences": occurrences, "first_seen": when,
            "last_seen": when}


def run(map_id=WAILING, state="active", started=None, ended=None, **kw):
    row = {"id": 1, "leader_name": "Grug", "map_id": map_id, "state": state,
           "started_at": started or datetime(2026, 9, 5, 13, 49, 49),
           "last_progress_at": datetime(2026, 9, 5, 13, 57, 33),
           "ended_at": ended, "ended_reason": "", "outcome": ""}
    row.update(kw)
    return row


def snapshot(name, map_id=WAILING, health=100, maximum=100, combat=0,
             seen=None, level=25):
    return {"name": name, "level": level, "map_id": map_id, "health": health,
            "max_health": maximum, "in_combat": combat,
...
SCAN_ROWS = [
    equip("Ugga", 2169, "Buzzer Blade", datetime(2026, 9, 2, 15, 41),
          map_id=36, zone=1581, occurrences=4),
    equip("Ugga", 2169, "Buzzer Blade", datetime(2026, 9, 5, 11, 2),
          map_id=0, zone=10, occurrences=4),
    equip("Ugga", 2169, "Buzzer Blade", datetime(2026, 9, 5, 13, 51, 29),
          occurrences=3),
...
class TheEquipRecordIsARescan(unittest.TestCase):
    ...
    def test_the_earliest_row_for_a_pair_is_the_one_kept(self):
        ...
        self.assertEqual(firsts[("Ugga", 2169)]["first_seen"],
    ...
    def test_rows_of_other_kinds_are_not_treated_as_equips(self):
        ...
        rows = SCAN_ROWS + [{"character_name": "Og", "kind": "quest_accept",
                             "subject_id": 255, "first_seen": NOW}]
    ...
    def test_a_rescan_inside_the_run_is_not_that_runs_loot(self):
        ...
        loot = recap.run_loot(run(), recap.first_equips(SCAN_ROWS), ITEMS, {},
                              NOW, DUNGEONS, ZONES)
        self.assertEqual([line["name"] for line in loot],
                         ["Gloves of the Fang"])
    ...
    def test_the_window_rule_would_have_taken_both(self):
        ...
        window = {(row["character_name"], row["subject_id"]) for row in SCAN_ROWS
...
class WhatARunsLootIs(unittest.TestCase):
    def test_an_item_first_worn_on_another_map_is_not_this_runs(self):
        rows = [equip("Og", 6538, "Willow Robe", datetime(2026, 9, 5, 13, 52),
                      map_id=0, zone=10)]
        self.assertEqual(recap.run_loot(run(), recap.first_equips(rows), {}, {},
                                        NOW, DUNGEONS, ZONES), [])
    ...
    def test_an_ended_runs_window_closes_at_its_end(self):
        ...
        self.assertEqual(recap.run_loot(ended, recap.first_equips(rows), {}, {},
                                        NOW, DUNGEONS, ZONES), [])
...
class WherePlacesGetTheirNames(unittest.TestCase):
    def test_a_dungeon_map_is_named_by_the_dungeon_table(self):
        self.assertEqual(recap.place_name(43, 718, DUNGEONS, ZONES),
                         "Wailing Caverns")
    ...
    def test_an_unknown_place_prints_its_ids_rather_than_shrugging(self):
        ...
        self.assertEqual(recap.place_name(0, 1581, DUNGEONS, ZONES),
                         "map 0, zone 1581")
    ...
    def test_zone_names_come_out_of_the_zones_file_shape(self):
        continents = {"0": {"zones": [{"area_id": 10, "name": "Duskwood"}]},
                      "1": {"zones": [{"area_id": 17, "name": "Barrens"}]}}
        self.assertEqual(recap.zone_names(continents),
                         {10: "Duskwood", 17: "Barrens"})


class HowFarThroughTheyAre(unittest.TestCase):
    ENCOUNTERS = [{"entry": 591, "creditEntry": 5775, "name": "Verdan"},
    ...
    def test_the_encounters_come_back_in_the_tables_own_order(self):
        self.assertEqual([e["name"] for e in recap.encounter_order(self.ENCOUNTERS)],
    ...
    def test_the_basis_admits_the_bit_order_is_inferred(self):
        ...
        basis = recap.encounters_down(7, recap.encounter_order(self.ENCOUNTERS))["basis"]
...
class WhichInstanceRowIsThisRuns(unittest.TestCase):
    ROWS = [{"id": 2, "map": 43, "completedEncounters": 7},
...
class WhoIsInThere(unittest.TestCase):
    ...
    def test_a_member_elsewhere_is_not(self):
        state = recap.party_state(["Grug"], [snapshot("Grug", map_id=1)],
                                  run(), NOW)[0]
    ...
    def test_zero_health_is_down_and_nothing_cleverer(self):
        ...
        state = recap.party_state(["Grug"], [snapshot("Grug", health=0)],
                                  run(), NOW)[0]
        self.assertFalse(state["alive"])
        self.assertEqual(state["note"], "down")

    def test_fighting_is_said_when_the_snapshot_says_so(self):
        state = recap.party_state(["Grug"], [snapshot("Grug", combat=1)],
                                  run(), NOW)[0]
    ...
    def test_every_member_carries_a_tone_the_page_can_paint_from(self):
        ...
        rows = [snapshot("Bork"), snapshot("Grog", combat=1),
                snapshot("Grug", health=0), snapshot("Og", map_id=1)]
        tones = {m["name"]: m["tone"] for m
                 in recap.party_state(["Bork", "Grog", "Grug", "Og", "Ugga"],
                                      rows, run(), NOW)}
        self.assertEqual(tones, {"Bork": "", "Grog": "fighting",
                                 "Grug": "dead", "Og": "away", "Ugga": "away"})

    def test_a_stale_snapshot_is_toned_as_away_rather_than_as_present(self):
        old = snapshot("Grug", seen=NOW - timedelta(minutes=4))
        self.assertEqual(recap.party_state(["Grug"], [old], run(), NOW)[0]["tone"],
                         "away")
...
class TheRecapWhenSomethingIsHappening(unittest.TestCase):
    def build(self, **kw):
        args = dict(run_rows=[run()], event_rows=SCAN_ROWS, death_rows=[],
                    ...
                    roster=ROSTER, items=ITEMS, icons={}, dungeons=DUNGEONS,
                    zones=ZONES, now=NOW)
        args.update(kw)
    ...
    def test_the_party_line_counts_the_fallen_separately(self):
        ...
        self.assertEqual(self.build(snapshot_rows=rows)["party_line"],
    ...
    def test_only_deaths_inside_the_run_and_on_its_map_are_counted(self):
        deaths = [
            {"character_name": "Ugga", "map": 43, "zone": 718,
             "killer_name": "Skum", "killer_type": "creature",
             "created_at": datetime(2026, 9, 5, 13, 55)},
            {"character_name": "Og", "map": 0, "zone": 10,
             "killer_name": "a wolf", "killer_type": "creature",
             "created_at": datetime(2026, 9, 5, 13, 55)},
        ]
...
class TheRecapWhenNothingIsHappening(unittest.TestCase):
    ...
    def build(self, run_rows):
        return recap.build_recap(
            run_rows=run_rows, event_rows=SCAN_ROWS, death_rows=[],
...
class WhichMapTheBoardIsFor(unittest.TestCase):
    ...
    def test_then_the_most_recent_runs_map(self):
        rows = [run(map_id=36, state="ended", ended=NOW),
                run(map_id=209, state="ended", ended=NOW,
                    started=datetime(2026, 9, 1, 1, 1))]
...
def drop(entry, name, ilvl, inv_type, klass=4, subclass=2, required=0,
         chance=50.0, creature=3669, allowable=-1, group=1):
    return {"Entry": 3669, "Item": entry, "Chance": chance, "GroupId": group,
            "creature": creature, "item_name": name, "quality": 3,
            "item_level": ilvl, "required_level": required, "class": klass,
            "subclass": subclass, "inventory_type": inv_type,
            "displayid": None, "allowable_class": allowable}
...
def worn(who, slot, name, ilvl, klass=4, subclass=2, inv_type=5):
    return {"name": who, "slot": slot, "entry": 900 + slot, "item_name": name,
            ...
            "subclass": subclass, "inventory_type": inv_type, "displayid": None}
...
class WhetherADropBeatsWhatIsWorn(unittest.TestCase):
    ...
    def test_a_lower_item_level_is_not_dressed_up(self):
        self.assertEqual(
            recap.verdict(drop(1, "Rag", 12, 5), self.member())["verdict"],
            recap.WORSE)
    ...
    def test_armour_too_heavy_for_what_they_wear_is_refused(self):
        ...
        actually has today."""
        v = recap.verdict(drop(1, "Plate Chest", 40, 5, subclass=4),
                          self.member())
    ...
    def test_armour_no_heavier_than_what_they_wear_is_ranked_normally(self):
        v = recap.verdict(drop(1, "Cloth Robe", 23, 5, subclass=1),
                          self.member())
        self.assertEqual(v["verdict"], recap.UPGRADE)

    def test_a_level_they_have_not_reached_is_said_with_both_numbers(self):
        v = recap.verdict(drop(1, "Big Chest", 40, 5, required=30),
                          self.member(level=25))
    ...
    def test_a_worn_item_the_world_table_cannot_name_is_not_ranked(self):
        ...
        item worn there"."""
        gear = [dict(worn("Grug", 4, "Old Tunic", 20), item_name=None,
                     item_level=None)]
        v = recap.verdict(drop(1, "Armor of the Fang", 23, 5),
                          self.member(gear=gear))
        self.assertEqual(v["verdict"], recap.UNRANKED)
        self.assertIn("does not know the item worn", v["why"])

    def test_an_unnamed_item_is_the_one_a_ring_would_displace(self):
        gear = [dict(worn("Grug", 10, "Good Ring", 22, inv_type=11)),
                dict(worn("Grug", 11, "Mystery", 0, inv_type=11),
                     item_level=None)]
        v = recap.verdict(drop(1, "Heart Ring", 18, 11), self.member(gear=gear))
        self.assertEqual(v["verdict"], recap.UNRANKED)

    def test_a_bag_is_not_ranked_at_all(self):
        v = recap.verdict(drop(1, "Snakeskin Bag", 25, 18, klass=1, subclass=0),
                          self.member())
        self.assertEqual(v["verdict"], recap.UNRANKED)
    ...
    def test_the_armour_grade_is_the_heaviest_piece_worn(self):
        gear = [worn("Grug", 4, "Mail Chest", 20, subclass=3),
    ...
    def test_a_shield_does_not_count_as_an_armour_grade(self):
        self.assertIsNone(recap.armour_grade([worn("Grug", 16, "Shield", 20,
                                                   subclass=6)]))


class TheBoardAsAWhole(unittest.TestCase):
    ENCOUNTERS = [{"entry": 585, "creditEntry": 3671, "name": "Lady Anacondra"},
                  {"entry": 586, "creditEntry": 3669, "name": "Lord Cobrahn"}]
    LOOT = [drop(10410, "Leggings of the Fang", 23, 7, creature=3669),
            drop(6465, "Robe of the Moccasin", 22, 20, subclass=1, creature=3669),
            drop(5404, "Serpent's Shoulders", 23, 3, creature=3671),
            drop(6446, "Snakeskin Bag", 25, 18, klass=1, subclass=0,
                 creature=3671)]
    CHARS = [{"name": "Grug", "level": 29, "class": 1}]
    WORN = [worn("Grug", 4, "Defender Tunic", 23),
            worn("Grug", 2, "Old Pads", 18, inv_type=3)]

    def board(self):
        return recap.build_lootboard(43, "Wailing Caverns", self.ENCOUNTERS,
                                     self.LOOT, self.CHARS, self.WORN, {},
                                     ["Grug"])

    def test_the_bosses_are_the_encounter_tables_and_in_its_order(self):
        self.assertEqual([b["name"] for b in self.board()["bosses"]],
    ...
    def test_the_boss_line_counts_what_is_worth_taking(self):
        self.assertEqual(self.board()["bosses"][0]["line"],
                         "1 pieces, 1 of them worth taking")
    ...
    def test_a_map_with_no_encounters_says_so_rather_than_drawing_nothing(self):
        empty = recap.build_lootboard(1, "Kalimdor", [], [], self.CHARS,
                                      self.WORN, {}, ["Grug"])
    ...
    def test_a_caveat_names_no_member_because_it_is_about_the_item(self):
        ...
        weapon = recap.build_lootboard(
            43, "Wailing Caverns", self.ENCOUNTERS,
            [drop(6469, "Venomstrike", 24, 15, klass=2, subclass=2,
                  creature=3669)],
            self.CHARS, self.WORN, {}, ["Grug"])
    ...
    def test_a_shared_roll_reports_its_group_and_not_a_percentage(self):
        ...
        board = recap.build_lootboard(43, "Wailing Caverns", self.ENCOUNTERS,
                                      shared, self.CHARS, self.WORN, {}, ["Grug"])
...
class WhereAWornItemCameFrom(unittest.TestCase):
    EQUIPPED = [{"name": "Ugga", "slot": 15, "entry": 2169},
    ...
    def test_the_line_names_the_date_the_record_starts(self):
        self.assertIn("02 September 2026",
                      self.index()["Ugga"][4444]["line"])
    ...
    def test_the_record_start_is_the_oldest_equip(self):
        self.assertEqual(recap.record_starts(SCAN_ROWS),
                         datetime(2026, 9, 2, 15, 41))
...
class TheHouseRules(unittest.TestCase):
    def test_no_em_dashes(self):
        for name in ("recap.py", "achievements.py", "map_server.py",
                     "index.html", "tests/test_recap.py",
