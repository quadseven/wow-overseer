...
PILES = {
    JUNK: ("junk", "sold at the next vendor trip, unless an earlier refusal "
           "for want of a vendor is still holding it (up to a day)", GOOD, 149),
    FOR_RELATIVE: ("gear for {who}", "handed over by the family gear pass "
                   "once {who} is close by with a free slot", GOOD, None),
    AUCTION: ("gear nobody in the family would wear, still tradable",
              "listed when the leader reaches an auctioneer, which the "
              "shared travel slot rarely allows", CAUTION, 148),
    VENDOR_GEAR: ("plain gear nobody in the family would wear",
                  "sold at the next vendor trip, unless an earlier refusal "
                  "is still holding it (up to a day)", GOOD, 149),
    FOR_HOLDER: ("upgrades for {who}, carried and not worn",
                 "kept - nothing puts on a carried upgrade", ALARM, 146),
    DUST: ("green or better gear nobody would wear, and bound",
    ...
    UNJUDGED: ("gear the family check cannot judge (rings, trinkets, relics)",
               "kept - not offered to a guildmate either", CAUTION, 148),
    MISC_PILE: ("lockboxes and other oddments", "kept - no pass routes them",
                CAUTION, 148),
    TRADE: ("trade goods and reagents", "kept for a family trade, or shared "
            "with a guildmate who needs them", PLAIN, None),
    TOOLS: ("trade tools", "kept for the family's trades, never sold",
            PLAIN, None),
    CONSUMABLES: ("consumables", "kept to be used", PLAIN, None),
    BAGS: ("spare bags", "equipped by the bag pass, or sold as a spare",
           PLAIN, None),
...
TRIP_WAIT = ("no vendor trip until somebody is down to %d free slots"
             % bag_pressure.TOWN_RUN_FREE_SLOTS)
HEADING = "where it is going"
NOTE = ("Each carried stack in one pile. A pile that goes nowhere says why, "
...
def pile_of(row: dict, claimant: str | None, holder: str) -> tuple[str, str]:
    ...
    if disposition.trade_tool(disposition.Item(
            name="", item_class=item_class or 0,
...
def family_claims(carried: dict[str, list[dict]], equipped: dict[str, list[dict]],
                  levels: dict[str, tuple[int, int]]) -> dict[int, str]:
    ...
    for name, rows in carried.items():
        for r in rows:
            if r.get("class") not in GEAR_CLASSES:
                continue
            gear_rows.append({
                "holder": name, "item_guid": r.get("item_guid"),
                "entry": r.get("entry"), "name": r.get("item_name") or "",
                "quality": r.get("quality"), "item_level": r.get("item_level"),
...
def build_fates(holder: str, carried_rows: list[dict], claims: dict[int, str],
                free_slots: int | None, managed: bool) -> dict:
    """One character's piles, in the page's order, with every word composed."""
    if not managed:
        return {"heading": HEADING, "note": NOTE, "unmanaged": UNMANAGED,
                "piles": []}
    piles: dict[tuple[str, str], dict] = {}
    for row in carried_rows:
        key, who = pile_of(row, claims.get(int(row.get("item_guid") or 0)), holder)
        pile = piles.setdefault((key, who), {"key": key, "who": who,
                                             "stacks": 0, "items": []})
        pile["stacks"] += 1
    ...
    for key, who in sorted(piles, key=lambda k: (ORDER.index(k[0]), k[1])):
        ...
        blocker = None
        if key == JUNK and free_slots is not None \
        ...
        out.append({
            ...
            "count": "%d stack%s" % (pile["stacks"], "" if pile["stacks"] == 1 else "s"),
            "stacks": pile["stacks"],
